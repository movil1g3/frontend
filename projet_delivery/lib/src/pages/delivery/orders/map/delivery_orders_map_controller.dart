...
class DeliveryOrdersMapController extends GetxController {
  ...
  CameraPosition initialPosition = CameraPosition(
      target: LatLng(14.6562046, -86.2511316),
      zoom: 14
  );
  ...
  void isCloseToDeliveryPosition() {
    ...
    if (position != null) {
      ...
      if (distanceBetween <= 300 && isClose == false) {
        isClose = true;
        update();
      }
  ...
  void updateLocation() async {
    try{
      ...
      saveLocation();
      animateCameraPosition(position?.latitude ?? 14.6562046, position?.longitude ?? -86.2511316);

      addMarker(
          'delivery',
          position?.latitude ?? 14.6562046,
          position?.longitude ?? -86.2511316,
          'Tu posicion',
          '',
          deliveryMarker!
      );
      ...
      addMarker(
          'home',
          order.address?.lat ?? 14.6562046,
          order.address?.lng ?? -86.2511316,
          'Lugar de entrega',
          '',
          homeMarker!
      );
      ...
      LatLng to = LatLng(order.address?.lat ?? 14.6562046, order.address?.lng ?? -86.2511316);
